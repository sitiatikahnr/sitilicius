            ...
            <button type="submit" class="btn btn-default"><span class="glyphicon glyphicon-search"></span></button>
              ...
              <h3 style="margin:0px; padding:0px;">Hasil Cari</h3><hr>
                ...
                <tr><th>Kode Inventaris</th><th>Nama Barang</th><th>Status</th><th>Aksi</th></tr>
                                  ...
                                  $a = mysql_query("select * from detail_inventaris where kd_inventaris like '%$_POST[cari]%'");
                                  $hitung = mysql_num_rows($a);
                                  if($hitung==0){echo"Data yang anda cari tidak ada";}
                                  else{
                                    while($b = mysql_fetch_array($a)){
                                      $c = mysql_fetch_array(mysql_query("select * from alat where kd_alat='$b[kd_alat]'"));
                                      echo"
                                             <tr>
                                          ...
                                          ";
                                          if($b['status']==0){
                                            echo"<a href='../lib/kembali.php?kode=$b[kd_inventaris]&nama=$b[kd_alat]&id=$b[id]'><button type='button' class='btn btn-warning'>Dipinjam</button></a>";
                                          }
                                          else{
